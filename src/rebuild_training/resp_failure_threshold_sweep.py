...
def analyse() -> None:
    ...
    for cutoff in SF_RATIO_CUTOFFS:
        key = f"SF<{cutoff}"
        candidates[key] = {"pos_rows": 0, "pos_patients": 0,
                           "total_rows": 0, "total_patients": 0}
    ...
    for idx, path in enumerate(psv_files):
        ...
        for r in filled:
            ...
            fio2 = r.get("FiO2")
            resp = r.get("Resp")
            sf = (spo2 / fio2) if (spo2 is not None and fio2 is not None and fio2 > 0) else None
            sf_vals.append(sf)
        ...
        for spo2_cut in SPO2_CUTOFFS:
            for resp_cut in RESP_CUTOFFS:
                ...
                raw = [
                    (
                        spo2_vals[i] is not None and spo2_vals[i] < spo2_cut and
                        resp_vals[i] is not None and resp_vals[i] > resp_cut
                    )
                    for i in range(n)
                ]
                sus = sustained_flag(raw, SUSTAIN_HOURS)
        ...
        for sf_cut in [150, 165, 180, 200]:
            for spo2_cut in [87, 88]:
                for resp_cut in [28, 30]:
                    ...
                    raw = [
                        (
                            (sf_vals[i] is not None and sf_vals[i] < sf_cut) or
                            (spo2_vals[i] is not None and spo2_vals[i] < spo2_cut and
                             resp_vals[i] is not None and resp_vals[i] > resp_cut)
                        )
                        for i in range(n)
    ...
    for key, s in sorted(candidates.items(), key=lambda x: x[1]["pos_rows"]):
        ...
        status = "VIABLE" if viable else ("too broad" if row_rate > TARGET_HIGH else "too narrow")
        print(f"{key:<40} {row_rate:>5.2f}%  {pat_rate:>5.2f}%  {pos_pat:>7,}  {status}")
